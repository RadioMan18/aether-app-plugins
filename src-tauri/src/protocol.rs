...
impl PluginProtocol {
    ...
    pub fn handler<R: tauri::Runtime>(
        ...
        let candidate = plugins_dir.join(relative_path);
        ...
        match std::fs::read(&resolved) {
            Ok(data) => {
                ...
                tauri::http::Response::builder()
                    ...
                    .header("Content-Security-Policy", "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src 'self' data:; connect-src 'none';")
                    .body(data)
